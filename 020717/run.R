...
to.plot=c("circ", "myc.sig", "mek", "tak1", "neg.ras.dep", "pos.ras.dep", "wnt", "STAT1", "CXCL10", "CIITA", "IRF1", "CD247")
...
if(do.tcga.analysis) {
    ...
    neoantigen.cnts <- table(neoantigen.tbl$patientBarcode)

    ## Append the counts to the expression matrix
    tmp <- tcga_expr
    tmp <- rbind(tmp, neoantigens=rep(NA, ncol(tmp)))
    neoantigen.cnts <- neoantigen.cnts[names(neoantigen.cnts) %in% colnames(tcga_expr)]
    tmp["neoantigens",names(neoantigen.cnts)] <- unname(neoantigen.cnts)
    
    ##    res <- doAnalysis(tcga_expr, clin, "tcga-all", to.plot=c("neoantigens", to.plot), ylabels=c("neo", ylabels))
        res <- doAnalysis(tmp, clin, "tcga-all", to.plot=c("neoantigens", to.plot), ylabels=c("neo", ylabels))
