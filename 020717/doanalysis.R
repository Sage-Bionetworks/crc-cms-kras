...
plot.beeswarm.with.err <- function(df, x.name, y.name) {
    ...
    ## Use raw pvalue for univariate analysis
    ##        pval <- as.numeric(kras.tbl$apval[sti])
    pval <- as.numeric(wilcox.test(as.formula(paste(y.name, x.name, sep=" ~ ")), data = df)$p.value)
...
doAnalysis <- function(expr, clin, analysis.name, to.plot=c(), ylabels=c(), kras.status.field="kras", kras.states=c("MT","WT")) {
    ...
    ## b. tak vs circ
    ## c. wnt vs myc
    sigs1 <- c("wnt", "tak1", "wnt", "pos.ras.dep", "pos.ras.dep", "neg.ras.dep", "pos.ras.dep", "neg.ras.dep")
    sigs2 <- c("circ", "circ", "myc.sig", "neg.ras.dep", "circ", "circ", "tak1", "tak1")
...
do.msi <- function(expr, clin, file, mut.tbl = NULL, sample.col = "Tumor_Sample_Barcode", gene.col = "SYMBOL", variant.type.col = "Variant_Classification", num.clusters = 2) {
...
## out.png <- paste("output/", st, "-", analysis.name, "-kras-cms-faceted", ".png", sep="")
## univariate -cms.png
## univeriate -kras.png  <- problem
## forest
## -no-interaction
## look at fraction of CMS2 (and CMS3) in tcga and kfs

## cp circ-tcga-*kras-cms-faceted.png afigs/
## cp circ-kfs-*kras-cms-faceted.png afigs/
## cp circ-tcga-all-cms.png afigs/
## cp circ-tcga-mss-cms.png afigs/
## cp circ-tcga-mss-msi-inferred-cms.png afigs/
## cp circ-kfs-all-cms.png afigs/
## cp circ-kfs-mss-msi-inferred-cms.png afigs/

## cp circ-tcga-all-kras-MT-vs-WT.png afigs/
## cp circ-tcga-mss-kras-MT-vs-WT.png afigs/
## cp circ-tcga-mss-msi-inferred-kras-MT-vs-WT.png afigs/

## cp circ-kfs-all-kras-MT-vs-WT.png afigs/
## cp circ-kfs-mss-msi-inferred-kras-MT-vs-WT.png afigs/
