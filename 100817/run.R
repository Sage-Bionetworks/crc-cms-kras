...
if(final.submission) { 
    add.figure.labels <- FALSE
}
## Figures must be at least 300 dpi
dpi <- 300
## Width of one or two columns in inches
one.col.in <- 3
two.col.in <- 6.5

two.col.scale <- two.col.in / 14
one.col.scale <- one.col.in / 7
...
## g.bindea <- g.bindea + theme(text = element_text(size = 5)) 
## g.bindea <- g.bindea + theme(axis.text = element_text(size = 5))
## g.bindea <- g.bindea + theme(axis.title = element_text(size = 5))
...
##g.hall <- g.hall + theme(text = element_text(size = 5)) 
##g.hall <- g.hall + theme(axis.text = element_text(size = 5))
##g.hall <- g.hall + theme(axis.title = element_text(size = 5))
...
pg
## NB: default height and width are 7.  So, this is effectively making
## the width twice as large as the height.  Do the same below.
## ggsave("fig1.tiff", width = two.col.in, height = two.col.in / 2, dpi = dpi)
ggsave("fig1-full.tiff", width = 14, dpi = dpi)
cmd <- paste0("convert fig1-full.tiff -resize ", two.col.in * dpi, "x", two.col.in/2 * dpi, " fig1.tiff")
system(cmd)
...
g.infg
ggsave("supp-fig6-full.tiff", dpi = dpi)
cmd <- paste0("convert supp-fig6-full.tiff -resize ", one.col.in * dpi, "x", one.col.in * dpi, " supp-fig6.tiff")
system(cmd)
...
if(add.figure.labels) {
    pg <- pg + draw_label("Supp Figure 3", x=1, y=1, vjust=1, hjust=1)
}
pg
prefix <- "supp-fig3"
ggsave(paste0(prefix, "-full.tiff"), width = 14, dpi = dpi)
cmd <- paste0("convert ", prefix, "-full.tiff -resize ", two.col.in * dpi, "x", two.col.in/2 * dpi, " ", prefix, ".tiff")
system(cmd)
...
## ggsave("supp-fig4.tiff", width = two.col.in, height = two.col.in / 2, dpi = dpi)
prefix <- "supp-fig4"
ggsave(paste0(prefix, "-full.tiff"), width=14, dpi = dpi)
cmd <- paste0("convert ", prefix, "-full.tiff -resize ", two.col.in * dpi, "x", two.col.in/2 * dpi, " ", prefix, ".tiff")
system(cmd)

## Plot the pairwise correlation of the immune signatures and CIRC (Supp Figure 5)

## tiff("supp-fig5a-full.tiff", res = dpi)
setEPS()
postscript("supp-fig5a-full.eps")
...
##prefix <- "supp-fig5a"
##cmd <- paste0("convert ", prefix, "-full.tiff -resize ", one.col.in * dpi, "x", one.col.in * dpi, " ", prefix, ".tiff")
##system(cmd)

##tiff("supp-fig5b-full.tiff", res = dpi)
setEPS()
postscript("supp-fig5b-full.eps")
...
if(add.figure.labels) {
    mtext("Supp Fig 5", line=2.5, at = 1, cex = 1.5)
}
d <- dev.off()
...
## ggsave("supp-fig10.tiff", width = two.col.in, height = two.col.in / 2, dpi = dpi)
prefix <- "supp-fig7"
ggsave(paste0(prefix, "-full.tiff"), width = 14, dpi = dpi)
cmd <- paste0("convert ", prefix, "-full.tiff -resize ", two.col.in * dpi, "x", two.col.in/2 * dpi, " ", prefix, ".tiff")
system(cmd)
...
## ggsave("fig2.tiff", width = two.col.in, height = two.col.in / 2, dpi = dpi)
prefix <- "fig2"
ggsave(paste0(prefix, "-full.tiff"), width = 14, dpi = dpi)
cmd <- paste0("convert ", prefix, "-full.tiff -resize ", two.col.in * dpi, "x", two.col.in/2 * dpi, " ", prefix, ".tiff")
system(cmd)
...
##ggsave("fig3.tiff", width = two.col.in, height = two.col.in / 2, dpi = dpi)
prefix <- "fig3"
ggsave(paste0(prefix, "-full.tiff"), width = 14, dpi = dpi)
cmd <- paste0("convert ", prefix, "-full.tiff -resize ", two.col.in * dpi, "x", two.col.in/2 * dpi, " ", prefix, ".tiff")
system(cmd)
...
files <- c("fig4", "fig5", "supp-fig8")
labels <- c("Figure 4", "Figure 5", "Supp Figure 8")
labels <- c(NA, NA, "Supp Figure 8")
if(!add.figure.labels) {
    labels <- c(NA, NA, NA)
}
plots <- list()
...
for(i in 1:length(ns)) {
    ...
    if(name == "ifng") {
        ...
        filename <- paste0(files[i], "-full.tiff")
        ## ggsave(filename)
        prefix <- files[i]
        ggsave(paste0(prefix, "-full.tiff"), dpi = dpi)
        cmd <- paste0("convert ", prefix, "-full.tiff -resize ", one.col.in * dpi, "x", one.col.in * dpi, " ", prefix, ".tiff")
        system(cmd)
        
        ## ggsave(filename, width = one.col.in, height = one.col.in, res = dpi)
    }
