...
doAnalysis <- function(expr, clin, analysis.name, to.plot=c(), ylabels=c(), kras.status.field="kras", kras.score.field="kras", kras.states=c("MT","WT")) {
    ...
    cols <- c(cms1mt.vs.cms2mt.col, cms2mt.vs.wt.col, cms3mt.vs.cms2mt.col, cms4mt.vs.cms2mt.col, kras.col)
    for(prefix in cols) {
    ...
    for(sti in 1:length(to.plot)) {
        ...
        df <- data.frame(expr=esn.st, CMS=factor(cms, levels=c("CMS1", unique.cms[unique.cms != "CMS1"])), KRAS=factor(kras.label, levels=c("WT", "MT")))
        if(do.coarse.grained.analysis) {
            df <- data.frame(expr=esn.st, CMS=factor(cms, levels=c("CMSOther", unique.cms[unique.cms != "CMSOther"])), KRAS=factor(kras.label, levels=c("WT", "MT")))
        }
        ...
        for(i in 1:length(formulas)) {
            ...
            if(formula.names[i] == "kras") {
                ...
                cat(paste0("Attempting to write ", coeffs.2[kras.flag, 5], " to col ", col, " for ", sti, "\n"))
                tbl[sti, col] <- coeffs.2[kras.flag, 5]
            }
            if(formula.names[i] == "kras-cms-interaction") {
                flag <- grepl(x=coeffs.2$coefficient, pattern="KRASMT") & grepl(x=coeffs.2$coefficient, pattern="CMS2")
...
do.rasness.analysis.based.on.harmonized.training <- TRUE
