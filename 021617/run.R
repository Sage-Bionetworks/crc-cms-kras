...
obj <- synGet(id="syn2527101", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
clin.data.file <- getFileLocation(obj)
clin <- read.table(clin.data.file, sep=",", header=TRUE, as.is=TRUE)

suppressPackageStartupMessages(library("xlsx"))
ras.dependent.tbl <- read.xlsx(paste0(synapse.repo.dir, "/singh-mmc1.xlsx"), sheetName = "Sheet1")

ras.dependent.signature <- as.character(ras.dependent.tbl$SYMBOL)
ras.dependent.pos.signature <- as.character(ras.dependent.tbl$SYMBOL[as.numeric(as.character(ras.dependent.tbl$DEP.SCORE)) > 0])
ras.dependent.neg.signature <- as.character(ras.dependent.tbl$SYMBOL[as.numeric(as.character(ras.dependent.tbl$DEP.SCORE)) < 0])
...
obj <- synGet(id="syn2754865", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
...
bindeaTbl <- read.table(paste0(synapse.repo.dir, "/bindea_immune.txt"),sep='\t',header=TRUE)
...
circ.genes <- read.table(paste0(synapse.repo.dir, "/circ_sig.txt"),as.is=TRUE)[,1]
...
bindeaGsets[["pos.ras.dep"]] <- ras.dependent.pos.signature
bindeaGsets[["neg.ras.dep"]] <- ras.dependent.neg.signature    
...
cat("Reading clinical data\n")
obj <- synGet(id="syn2754862", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
...
neoantigen.tbl <- read.table(paste0(synapse.repo.dir, "/", "tcia-crc-neoantigens.tsv"), sep="\t", header=TRUE)
...
do.custom.affy <- function(synId){
  ...
  # data.set <- synId
    ...
    obj <- synGet(id=annotationSynId, downloadFile = TRUE, downloadLocation = synapse.repo.dir)
    file <- getFileLocation(obj)
...
do.discover.print.19742 <- function(synId) {
    ...
    obj <- synGet(id="syn2192791", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
...
do.discover.print.32627 <- function(synId) {
    ...
    obj <- synGet(id="syn2192801", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
    file <- getFileLocation(obj)    
    trans <- read.table(file, sep="\t", header=TRUE)
...
do.discover.print.mdacc <- function(synId) {
    ...
    obj <- synGet(id="syn2233216", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
    file <- getFileLocation(obj)    
    trans <- read.table(file, sep="\t", header=TRUE)
    trans <- trans[!duplicated(trans$ProbeName, fromLast=TRUE) & !duplicated(trans$ProbeName, fromLast=FALSE),]
    rownames(trans) <- trans$ProbeName
...
if(read.harmonized.data || do.harmonized.analysis) {
    ...
    obj <- synGet(id="syn2417854", downloadFile = TRUE, downloadLocation = synapse.repo.dir)
...
if(do.harmonized.rasness.analysis) {
    ...
    for(train.set in c("petacc", "tcga", "kfs")) {
        optimize.set <- train.set
        prefix=paste0("rasness-harm-", train.set, "-trained-", optimize.set, "-optimized")
        clin.rasness[[train.set]] <- fit.rasness.harmonized(clin, harmonized_expr, train.set, optimize.set, prefix, use.published.alpha = TRUE)
    }
...
if(do.tcga.analysis || do.kfs.analysis) {
  ...
  tcga_cibersort_mat <- read.table(paste0(synapse.repo.dir, "/CIBERSORT.Output_Job4_TCGA.tsv"), sep="\t", header=TRUE, as.is=TRUE)
##  plot.cibersort(tcga_cibersort_mat, clin, "tcga-cibersort")  
  ##  plot.cibersort.beeswarm(tcga_cibersort_mat, clin[!is.na(clin$cms_label) & (clin$cms_label == "CMS2"),], "tcga-cibersort-scatters-cms2")
  ...
  ## Look at correlations with MYC and WNT
  ## Neither MYC nor WNT are correlated with CIRC (visually, clearly not)
  ## As a sanity check WNT and MYC are strongly correlated
  if(FALSE) {
      res <- doAnalysis(tcga_expr, clin, "tcga-pathways", to.plot=genes.to.plot, ylabels=gene.labels, only.do.kras.analysis=TRUE)
      tcga.pathways <- res[["esn"]]
      plot(tcga.pathways["CIRC",], tcga.pathways["myc.sig",])
      plot(tcga.pathways["CIRC",], tcga.pathways["wnt",])
      plot(tcga.pathways["wnt",], tcga.pathways["myc.sig",])
      
      res <- doAnalysis(kfsyscc_expr, clin, "kfs-pathways", to.plot=genes.to.plot, ylabels=gene.labels, only.do.kras.analysis=TRUE)
      kfs.pathways <- res[["esn"]]
      plot(kfs.pathways["CIRC",], kfs.pathways["myc.sig",])
      plot(kfs.pathways["CIRC",], kfs.pathways["wnt",])
      plot(kfs.pathways["wnt",], kfs.pathways["myc.sig",])
  }
  ...
  flag <- merged.kras.tbl$mutUp.tcga == TRUE
  ...
  flag <- merged.kras.tbl$mutUp.kfs == TRUE
  ...
  pdf(paste0("output/gene-set-heatmap.pdf"), onefile=FALSE)
...
##  print(g)
  ...
  pdf(paste0("output/circ-gene-heatmap.pdf"), onefile=FALSE)  
  ...
  pdf(paste0("output/", "tcga-bindea-correlation.pdf"))  
  ...
  pdf(paste0("output/", "tcga-bindea-dendrogram.pdf"))    
  plot(hc, main=NULL, xlab=NA, sub=NA)
  d <- dev.off()
  
  pdf(paste0("output/", "tcga-bindea-dendrogram-all.pdf"))    
  ...
  pdf(paste0("output/", "kfs-bindea-correlation.pdf"))  
  pairs(t(esn[populations,]), upper.panel = panel.cor)
  d <- dev.off()
  ...
  dd <- as.dist((1 - cor(t(esn[populations,])))/2)
  hc <- hclust(dd, method="average")
  pdf(paste0("output/", "kfs-bindea-dendrogram.pdf"))    
  plot(hc, main=NULL, xlab=NA, sub=NA)
  d <- dev.off()
  
  pdf(paste0("output/", "kfs-bindea-dendrogram-all.pdf"))    
  methods <- c("ward.D", "ward.D2", "single", "complete", "average", "mcquitty", "median", "centroid")
  par(mfrow=c(2,4))
  for(method in methods) {
      hc <- hclust(dd, method=method)
  ...
  ## Plot correlation of circ with cibersort
  tmp <- tcga_cibersort_mat
  tmp$Input.Sample <- unlist(lapply(tmp$Input.Sample, function(x) gsub(x=x, pattern="\\.", replacement="-")))

  rownames(tmp) <- tmp$Input.Sample
  tmp <- tmp[,!(colnames(tmp) %in% c("Input.Sample", "P.value", "Pearson.Correlation", "RMSE"))]
  tmp <- t(tmp)
  
  common.samples <- intersect(colnames(tcga.esn), colnames(tmp))
  tmp <- t(rbind(tmp[,common.samples], tcga.esn["circ",common.samples,drop=F]))
  populations <- colnames(tmp)[!(colnames(tmp) %in% "circ")]
  tcga.correlations <- lapply(populations, function(population) {
      lm.obj <- lm(tmp[,"circ",drop=T] ~ tmp[,population,drop=T])
      sum.obj <- summary(lm.obj)
      f <- sum.obj$fstatistic
      pval <- pf(f[1],f[2],f[3],lower.tail=F)
      list(pval = unname(pval), r2 = sum.obj$r.squared)
  })
  tcga.correlations <- do.call("rbind", tcga.correlations)
  rownames(tcga.correlations) <- populations
...
## CD247 is PD-L1
## to.plot <- c("CIRC", "myc.sig", "mek", "tak1", "neg.ras.dep", "pos.ras.dep", "wnt", "STAT1", "CXCL10", "CIITA", "IRF1", "CD247", "neoantigens")
to.plot <- c("CIRC", "myc.sig", "mek", "tak1", "neg.ras.dep", "pos.ras.dep", "wnt",  "neoantigens", "STAT1", "CXCL10", "CIITA", "IRF1", "CD247")
to.plot <- c("CIRC", "myc.sig", "mek", "tak1", "neg.ras.dep", "pos.ras.dep", "wnt",  "neoantigens")
num.enrichment <- which(to.plot == "wnt")

## to.plot <- c("CIRC", "myc.sig", "wnt", "neoantigens", "STAT1")
## num.enrichment <- which(to.plot == "wnt")
...
neo <- which(to.plot == "neoantigens")
if(any(neo)) {
  ylabels[neo] <- "Log10 ( # Neoantigens + 1 )"
}
...
## to.plot <- c("CIRC", "neoantigens", "TNF", "IL6", "MDSC", "NFKB")
to.plot <- c("CIRC", "neoantigens", "IL1A", "IL1B", "IL10", "IL21", "VEGFA")
to.plot <- c("TGFB1", "TGFB2", "TGFB3", "IL4", "IL8", "IL11", "IL17A", "IL22", "IL23", "IL33")
ylabels <- paste(to.plot, " Enrichment Score", sep=" ")

for(gset in c("CELL_CYCLE_BIOCARTA", "KEGG_CELL_CYCLE", "SRC_ACT_BIOCARTA", "SRC_ACT_BILD", "MYC_TARGETS_ZELLER", "MYC_TARGETS_CP", "WNT_FLIER", "WNT_BIOCARTA", "MAPK_KEGG", "MAPK_ACT_GO_BP")) {
    bindeaGsets[[gset]] <- gene.sets$genesets[[which(gene.sets$geneset.names == gset)]]
}

to.plot <- c("CELL_CYCLE_BIOCARTA", "KEGG_CELL_CYCLE", "SRC_ACT_BIOCARTA", "SRC_ACT_BILD", "MYC_TARGETS_ZELLER", "MYC_TARGETS_CP", "WNT_FLIER", "WNT_BIOCARTA")

to.plot <- c("PDCD1", "CTLA4", "CD274", "PDCD1LG2", "CD80", "CD86")

to.plot <- c("B cells")

to.plot <- c("CD8A", "PTPRC")

to.plot <- c("MAPK_KEGG", "MAPK_ACT_GO_BP")
...
to.plot <- c("CIITA", "wnt", "myc.sig")
ylabels <- c("CIITA Expression", "Wnt Signature Enrichment Score", "Myc Signature Enrichment Score")


## to.plot <- c("tak1", "pos.ras.dep", "neg.ras.dep", "wnt", "CIRC", "myc.sig")
## ylabels <- paste(to.plot, "Enrichment Score", sep=" ")
...
do.agendia.ico208.analysis <- FALSE
ico208_expr <- NULL
...
do.agendia.vh70.analysis <- FALSE
vh70_expr <- NULL
...
do.merged.analysis <- FALSE
if(do.merged.analysis) {
...
if(do.tcga.analysis) {
    ...
    if(do.harmonized.rasness.analysis) {
        cat("Doing PETACC-trained RASness-based TCGA analysis\n")        
        res <- doAnalysis(tcga_expr, clin.rasness[["petacc"]], "tcga-petacc-trained-rasness-all", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }
    ...
    ##    clin.tcga.msi.inferred <- do.msi.mut.tbl(tcga_expr, clin, "tcga-msi", mut.tbl = tcga.mut.table)
    clin.tcga.msi.inferred <- do.msi(tcga_expr, clin, "tcga-msi", neoantigen.cnts = neoantigen.cnts)

    ## do.ras.dependency(tcga_expr, clin, ras.dependent.signature, file = "tcga-ras-dependent")

    tcga.samples <- clin.tcga.msi.inferred$dataset == "tcga"
    table(clin.tcga.msi.inferred$msi[tcga.samples], clin.tcga.msi.inferred$msi.inferred[tcga.samples])
    table(clin.tcga.msi.inferred$cms_label[tcga.samples], clin.tcga.msi.inferred$msi.inferred[tcga.samples])
...
##    res <- doCorrelationAnalysis(tcga_expr, clin, "tcga-all", to.plot=to.plot, ylabels=ylabels)    
    ...
    if(do.harmonized.rasness.analysis) {
        tmp <- clin.rasness[["petacc"]][,c("sample","rasness","score.rasness","rasness.and.mut")]
        tmp <- merge(tmp, clin.mss.inferred, by="sample")
        cat("Doing PETACC-trained RASness-based TCGA analysis MSS (inferred only)\n")
        
        clin.mss <- tmp[!is.na(tmp$msi) & (tmp$msi == "mss"),]        
        res <- doAnalysis(tcga_expr, clin.mss, "tcga-petacc-trained-rasness-mss-msi-inferred", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }
    ...
    if(do.harmonized.rasness.analysis) {
        cat("Doing PETACC-trained RASness-based TCGA analysis MSS (annotated only)\n")
        
        clin.mss <- clin.rasness[["petacc"]][!is.na(clin.rasness[["petacc"]]$msi) & (clin.rasness[["petacc"]]$msi == "mss"),]        
        res <- doAnalysis(tcga_expr, clin.mss, "tcga-petacc-trained-rasness-mss-msi-annotated", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }
...
do.harmonized.petacc.analysis <- FALSE
...
if(do.french.analysis) {
    ...
    if(do.harmonized.rasness.analysis) {
        cat("Doing PETACC-trained RASness-based french analysis\n")        
        res <- doAnalysis(french_expr, clin.rasness[["petacc"]], "french-petacc-trained-rasness-all", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }

    
    res <- doAnalysis(french_expr, clin, "french-all", to.plot=to.plot, ylabels=ylabels)

    clin.french.msi.inferred <- do.msi(french_expr, clin, "french-msi", num.clusters = 3)
...
##    stop("This isn't going to work--need to fix do.msi so that it handles outlier sample; i.e., when msi and mss don't segregate into 2 clusters")
    res <- doAnalysis(french_expr, clin.french.msi.inferred, "french-all-msi-inferred", to.plot=to.plot, ylabels=ylabels)    

    if(do.harmonized.rasness.analysis) {
        tmp <- clin.rasness[["petacc"]][,c("sample","rasness","score.rasness","rasness.and.mut")]
        tmp <- merge(tmp, clin.french.msi.inferred, by="sample")
        cat("Doing PETACC-trained RASness-based french analysis MSS (inferred only)\n")
        
        clin.mss <- tmp[!is.na(tmp$msi) & (tmp$msi == "mss"),]        
        res <- doAnalysis(french_expr, clin.mss, "french-petacc-trained-rasness-mss-msi-inferred", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }
    ...
    ## Restrict to MSS cases
    clin.mss <- clin[!is.na(clin$msi) & (clin$msi == "mss"),]
    res <- doAnalysis(french_expr, clin.mss, "french-mss-msi-inferred", to.plot=to.plot, ylabels=ylabels)    
...
#    write.table(frenchR$tbl1, file="Middleton_french_tbl1.xls",sep="\t",quote=FALSE,row.names=FALSE)
  ...
  french_cibersort_mat <- read.table(paste0(synapse.repo.dir,"/CIBERSORT.Output_Job3_French.tsv"), sep="\t", header=TRUE, as.is=TRUE)
...
if(do.kfs.analysis) {
    ...
    if(do.harmonized.rasness.analysis) {
        cat("Doing PETACC-trained RASness-based kfs analysis\n")        
        res <- doAnalysis(kfsyscc_expr, clin.rasness[["petacc"]], "kfs-petacc-trained-rasness-all", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }
    ...
    if(do.harmonized.rasness.analysis) {
        tmp <- clin.rasness[["petacc"]][,c("sample","rasness","score.rasness","rasness.and.mut")]
        tmp <- merge(tmp, clin.kfs.msi.inferred, by="sample")
        cat("Doing PETACC-trained RASness-based kfs analysis MSS (inferred only)\n")
        
        clin.mss <- tmp[!is.na(tmp$msi) & (tmp$msi == "mss"),]        
        res <- doAnalysis(kfsyscc_expr, clin.mss, "kfs-petacc-trained-rasness-mss-msi-inferred", to.plot=to.plot, ylabels=ylabels, kras.status.field="rasness", kras.states=c("high","low"))
    }
...
##        write.table(res[[tbl]], file=paste0("Middleton_", dset, "_", tbl.underscore, ".xls"), sep="\t", quote=FALSE, row.names=FALSE)
    ...
    kfs_cibersort_mat <- read.table(paste0(synapse.repo.dir, "CIBERSORT.Output_Job2_KFS.tsv"), sep="\t", header=TRUE, as.is=TRUE)
##    plot.cibersort(kfs_cibersort_mat, clin, "kfs-cibersort")

    flag <- !is.na(clin$cms_label) & !is.na(clin$kras) & (clin$cms_label == "CMS2")
##    plot.cibersort(kfs_cibersort_mat, clin[flag,], "kfs-cibersort-cms2")    

    ##    plot.cibersort.beeswarm(kfs_cibersort_mat, clin[!is.na(clin$cms_label) & (clin$cms_label == "CMS2"),], "kfs-cibersort-scatters-cms2")


  ## Plot correlation of circ with cibersort
  tmp <- kfs_cibersort_mat
  tmp$Input.Sample <- unlist(lapply(tmp$Input.Sample, function(x) gsub(x=x, pattern="\\.", replacement="-")))
  ...
  common.samples <- intersect(colnames(kfs.esn), colnames(tmp))
  tmp <- t(rbind(tmp[,common.samples], kfs.esn["CIRC",common.samples,drop=F]))
  populations <- colnames(tmp)[!(colnames(tmp) %in% "CIRC")]
  kfs.correlations <- lapply(populations, function(population) {
      lm.obj <- lm(tmp[,"CIRC",drop=T] ~ tmp[,population,drop=T])
      sum.obj <- summary(lm.obj)
      f <- sum.obj$fstatistic
      pval <- pf(f[1],f[2],f[3],lower.tail=F)
  ...
  kfs.correlations <- do.call("rbind", kfs.correlations)
  rownames(kfs.correlations) <- populations

    
    cibersort.correlations <- merge(tcga.correlations, kfs.correlations, suffixes = c(".tcga", ".kfs"), by = "row.names")
    rownames(cibersort.correlations) <- cibersort.correlations$Row.names
    cibersort.correlations <- cibersort.correlations[,colnames(cibersort.correlations) != "Row.names"]
}
...
immune.sigs <- list()
immune.sigs[["CIRC"]] <- read.table(paste0(synapse.repo.dir, "circ_sig.txt"),as.is=TRUE)[,1]
for(immune.set in immune.sets) {
