...
## RAS-dependent signature taken from Singh Table S1
...
ras.dependent.signature <- as.character(ras.dependent.tbl$SYMBOL[as.numeric(as.character(ras.dependent.tbl$DEP.SCORE)) > 0])
ras.independent.signature <- as.character(ras.dependent.tbl$SYMBOL[as.numeric(as.character(ras.dependent.tbl$DEP.SCORE)) < 0])
bindeaGsets[["kras.dep"]] <- ras.dependent.signature
bindeaGsets[["kras.ind"]] <- ras.independent.signature
bindeaGsets[["kras.all"]] <- unique(c(ras.dependent.signature, ras.independent.signature))
...
## Genes in WNT that were not in TAK1
tak1.genes <- bindeaGsets[["tak1"]]
wnt.genes <- bindeaGsets[["wnt"]]
wnt.not.tak1 <- wnt.genes[!(wnt.genes %in% tak1.genes)]
bindeaGsets[["wnt.not.tak1"]] <- wnt.not.tak1

## Cleevers wnt taken from Table 1 of
## http://www.sciencedirect.com/science/article/pii/S0016508506017999?via%3Dihub#sec5

wnt.cleevers <- c("ASCL2", "AXIN2", "BMP4", "C1orf33", "HIG2", "HSPC111", "KITLG", "LGR5", "MYC", "NOL1", "PPIF", "SOX4", "WDR71", "ZIC2", "ZNRF3")
## HIG2 is HILPDA
## HSPC111 is NOP16
## NOL1 is NOP2
## WDR71 is PAAF1
## C1orf33 is MRTO4
wnt.cleevers <- c("ASCL2", "AXIN2", "BMP4", "MRTO4", "HILPDA", "NOP16", "KITLG", "LGR5", "MYC", "NOP2", "PPIF", "SOX4", "PAAF1", "ZIC2", "ZNRF3")
bindeaGsets[["wnt.cleevers"]] <- wnt.cleevers
...
write.table(file="kfsyscc_expr.tsv", kfsyscc_expr, row.names=TRUE, col.names=TRUE, sep="\t", quote=FALSE)

to.plot <- c("CIRC", "neoantigens", "BATF3", "THBD", "CCL4", "ATF3", "HALLMARK_INTERFERON_GAMMA_RESPONSE", "REACTOME_INTERFERON_GAMMA_SIGNALING", "IFNG", "mek")
ylabels <- c("CIRC Enrichment Score", "Log10 ( # Neoantigens + 1 ) ", "BATF3 Expression", "THBD Expression", "CCL4 Expression", "ATF3 Expression", "Hallmark IFNG Enrichment Score", "Reactome IFNG Enrichment Score", "IFNG Expression", "MEK Enrichment Score")
...
genes.of.interest <- c("ERV3-1", "ERV3-2", "ERV9-1", "ERVFH21-1", "ERVFRD-1", "ERVH-1", "ERVH-3", "ERVH-4", "ERVH-6", "ERVH48-1", "ERVK13-1", "ERVK3-2", "ERVMER34-1", "ERVMER61-1", "ERVV-1", "ERVW-1", "IL28A", "IL28B", "IL28RA", "ZNF354B", "IFIH1", "DDX58", "DHX58", "DNMT1", "IRF1")

genes.of.interest.labels <- unlist(lapply(genes.of.interest, function(str) paste0(str, " Expression")))

other.genes <- c("STAT1", "CIITA", "CXCL10", "IL6", "IFNG", "CD247", "BAMBI", "BMP7", "INHBB", genes.of.interest)
...
## DNMT1, IL28, IL29, IRF7, ZFP354B, MDA5, RIG1, LGP2, and ERV1-3

## IL28 = IL28A, IL28B, and/or IL28RA
## ZFP354B (is this a mouse gene) = ZNF354B (http://www.genecards.org/cgi-bin/carddisp.pl?gene=ZNF354B) 
## MDA5 = IFIH1 (http://www.genecards.org/cgi-bin/carddisp.pl?gene=IFIH1)
## RIG1 = DDX58 (http://www.genecards.org/cgi-bin/carddisp.pl?gene=DDX58)
## LGP2 = DHX58 (http://www.genecards.org/cgi-bin/carddisp.pl?gene=DHX58)
## ERV1-3 is this 3 genes?
## ERV1 = GFER (http://www.genecards.org/cgi-bin/carddisp.pl?gene=GFER)
## ERV2 = ?
## ERV3 = ?

## Is IL28 IL28A, IL28B,

## ZFP354B is ZNF354B in human?


other.genes <- c(other.genes, "kras.dep", "kras.ind", "kras.all", "tak1", "mek", "wnt", "wnt.cleevers")
other.labels <- c(other.labels, "KRAS DEP Signature Enrichment Score", "KRAS IND Signature Enrichment Score", "KRAS DEP + IND Signature Enrichment Score", "TAK1 Signature Enrichment Score", "MEK Enrichment Score", "WNT Signature Enrichment Score", "WNT (Cleevers) Signature Enrichment Score")
...
for(dataset in c("tcga", "kfs")) {
    ...
    kras.within.cms.res[[dataset]] <- res$tests
    kras.within.cms.tbl <- res$tests
    write.table(file = paste0(dataset, "-immune-hallmark-kras-within-cms.tsv"), kras.within.cms.tbl, sep="\t", row.names=FALSE, col.names=TRUE, quote=FALSE)
...
to.plot <- c(to.plot, "STAT1", "CXCL10", "CIITA", "HALLMARK_INTERFERON_GAMMA_RESPONSE", "kras.dep", "kras.ind", "kras.all", "tak1", "mek", "BMP7", "BAMBI", "INHBB")
ylabels <- c(ylabels, "STAT1 Expression", "CXCL10 Expression", "CIITA Expression", "INTERFERON_GAMMA_RESPONSE Enrichment Score", "KRAS DEP Enrichment Score", "KRAS IND Enrichment Score", "KRAS DEP + IND Enrichment Score", "TAK1 Enrichment Score", "MEK Enrichment Score", "BMP7 Expression", "BAMBI Expression", "INHBB Expression")

to.plot <- c("wnt", genes.of.interest)
ylabels <- c("WNT Enrichment Score", genes.of.interest.labels)
...
for(gene in genes.of.interest) {
    ...
    if(add.figure.labels) {
        pg <- pg + draw_label(paste0("Figure ", gene), x=1, y=1, vjust=1, hjust=1)
    }
    pg
    ggsave(paste0("fig-gene-", gene, "-kras.tiff"), width=14)
}

## Plot Singh KRAS dependency and TAK1 signatures vs WNT for KRAS MT and WT
sigs1 <- c("tak1", "kras.dep", "kras.ind", "kras.all")
sigs2 <- c("wnt", "wnt", "wnt", "wnt")
...
for(dataset in c("tcga", "kfs")) {
    ...
    es <- gsva.es.lst[[dataset]]
    clin.es <- clin.es.lst[[dataset]]
    cat(paste0("Breakdown of KRAS mutation by CMS type in ", dataset, "\n"))
    print(table(clin.es$kras, clin.es$cms_label))
    
    kras.states <- unique(clin.es$kras)
    cms.states <- unique(clin.es$cms_label)
    kras.states <- kras.states[!is.na(kras.states)]
    cms.states <- cms.states[!is.na(cms.states) & (cms.states != "NOLBL")]
    
    for(i in 1:length(sigs1)) {
        sig1 <- sigs1[i]
        sig2 <- sigs2[i]
        glist <- list()
        for(kras.state in kras.states) {
            flag <- !is.na(clin.es$kras) & (clin.es$kras == kras.state)
            g <- plot.correlation(x = es[sig1, flag], y = es[sig2, flag], x.name = toupper(sig1), y.name = toupper(sig2))
            ## df <- data.frame(x = es[sig1, flag], y = es[sig2, flag])
            ...
            ## g <- g + xlab(toupper(sig1))
            ## g <- g + ylab(toupper(sig2))
            glist[[length(glist)+1]] <- g
        }
        out.tif <- paste("scatter-", dataset, "-", sig1, "-vs-", sig2, "-kras.tif", sep="")
        tiff(out.tif)
        do.call("grid.arrange", c(glist, "top" = paste0(dataset, " ", sig1, " vs ", sig2)))
        d <- dev.off()
    }
}
...
##ylabels <- c(ylabels, "STAT1 Expression", "CXCL10 Expression", "CIITA Expression", "INTERFERON_GAMMA_RESPONSE Enrichment Score")

to.plot <- c(to.plot, "STAT1", "CXCL10", "CIITA", "HALLMARK_INTERFERON_GAMMA_RESPONSE", "kras.dep", "kras.ind", "kras.all", "tak1", "mek", "BMP7", "BAMBI", "INHBB", "wnt", "wnt.cleevers")
ylabels <- c(ylabels, "STAT1 Expression", "CXCL10 Expression", "CIITA Expression", "INTERFERON_GAMMA_RESPONSE Enrichment Score", "KRAS DEP Enrichment Score", "KRAS IND Enrichment Score", "KRAS DEP + IND Enrichment Score", "TAK1 Enrichment Score", "MEK Enrichment Score", "BMP7 Expression", "BAMBI Expression", "INHBB Expression", "WNT Signature Enrichment Score", "WNT (Cleevers) Signature Enrichment Score")
...
for(gene in genes.of.interest) {
    ...
    pg
    ggsave(paste0("fig-gene-", gene, "-kras-vs-cms.tiff"), width=14)
}
...
es <- gsva.es.lst[[dataset]]
clin.es <- clin.es.lst[[dataset]]
kras.within.cms.tbl <- kras.within.cms.res[[dataset]]
p.tcga <- plotKrasCMSWithinCMSAnalysis(es, clin.es, kras.within.cms.tbl, to.plot=to.plot, ylabels=ylabels, analysis.name = dataset, plot.adjusted.pvalues = FALSE, plot.pvals.as.stars = TRUE, main = ifelse(dataset == "tcga", "TCGA", "KFSYSCC"))
...
clin.es <- clin.es.lst[[dataset]]
kras.within.cms.tbl <- kras.within.cms.res[[dataset]]
p.kfs <- plotKrasCMSWithinCMSAnalysis(es, clin.es, kras.within.cms.tbl, to.plot=to.plot, ylabels=ylabels, analysis.name = dataset, plot.adjusted.pvalues = FALSE, plot.pvals.as.stars = TRUE, main = ifelse(dataset == "tcga", "TCGA", "KFSYSCC"))
...
for(gene in genes.of.interest) {
    ...
    pg
    ggsave(paste0("fig-gene-", gene, "-kras-within-cms.tiff"), width=14)
}




## Plot Singh KRAS dependency and TAK1 signatures vs WNT for KRAS x CMS
sigs1 <- c("tak1", "kras.dep", "kras.ind", "kras.all")
sigs2 <- c("wnt", "wnt", "wnt", "wnt")
...
for(dataset in c("tcga", "kfs")) {
    ...
    cms.states <- unique(clin.es$cms_label)
    kras.states <- kras.states[!is.na(kras.states)]
    cms.states <- sort(cms.states[!is.na(cms.states) & (cms.states != "NOLBL")])
    ...
    for(i in 1:length(sigs1)) {
        ...
        glist <- list()
        for(kras.state in kras.states) {
            for(cms.state in cms.states) {
                flag <- !is.na(clin.es$kras) & (clin.es$kras == kras.state)
                flag <- flag & !is.na(clin.es$cms_label) & (clin.es$cms_label == cms.state)
                g <- plot.correlation(x = es[sig1, flag], y = es[sig2, flag], x.name = toupper(sig1), y.name = toupper(sig2))
                ## df <- data.frame(x = es[sig1, flag], y = es[sig2, flag])
                ## g <- ggplot(data = df, aes(x = x, y = y))
                ## g <- g + geom_point()
                if(kras.state == 0) {
                    g <- g + ggtitle(paste0(cms.state, " KRAS WT"))
                } else {
                    g <- g + ggtitle(paste0(cms.state, " KRAS MT"))
                }
                ## g <- g + xlab(toupper(sig1))
                ## g <- g + ylab(toupper(sig2))
                glist[[length(glist)+1]] <- g
        ...
        out.tif <- paste("scatter-", dataset, "-", sig1, "-vs-", sig2, "-kras-cms.tif", sep="")
        tiff(out.tif)
        do.call("grid.arrange", c(glist, "top" = paste0(dataset, " ", sig1, " vs ", sig2)))
        d <- dev.off()
    }
...
##  for x in `ls *.tiff`; do echo $x; sips -s format pdf --out `basename $x .tiff`.pdf $x; done
## "/System/Library/Automator/Combine PDF Pages.action/Contents/Resources/join.py" --output kras-analyses.pdf fig-gene-*kras.pdf
## "/System/Library/Automator/Combine PDF Pages.action/Contents/Resources/join.py" --output kras-within-cms-analyses.pdf fig-gene-*kras-within-cms.pdf
## "/System/Library/Automator/Combine PDF Pages.action/Contents/Resources/join.py" --output kras-vs-cms-analyses.pdf fig-gene-*kras-vs-cms.pdf
