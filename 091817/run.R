...
other.genes <- c("STAT1", "CIITA", "CXCL10", "IRF1", "IL6", "IFNG", "CD247", "BAMBI", "BMP7", "INHBB")
...
for(gene in c("kras.dep", "kras.ind", "kras.all", "tak1", "mek", "BMP7", "BAMBI", "INHBB", "wnt.cleevers", "wnt")) {
    pg <- plot_grid(p.tcga[[gene]], p.kfs[[gene]], labels = c("A", "B"), nrow=1)
    if(add.figure.labels) {
        pg <- pg + draw_label(paste0("Figure ", gene), x=1, y=1, vjust=1, hjust=1)
    }
    pg
    ggsave(paste0("fig-", gene, "-kras-vs-cms.tiff"), width=14)
}
...
for(gene in c("kras.dep", "kras.ind", "kras.all", "tak1", "mek", "BMP7", "BAMBI", "INHBB", "wnt.cleevers", "wnt")) {
    ...
    pg
    ggsave(paste0("fig-", gene, "-kras-within-cms.tiff"), width=14)
}
