...
more run.out| grep -i fisher | grep 'CIRC vs neoantigens' | grep tcga-all | grep -v msi
## tcga-all CIRC vs neoantigens: tcga-all: Analyzing CIRC vs neoantigens using Fisher's Exact Test for Count Data: p = 2.77695954746732e-11 n = 342
...
more run.out | grep -i neoantigen | grep tcga-all | grep 'CMS2 vs CMS4' | grep -v msi | grep -v 'KRAS MT'
## tcga-all: Analyzing neoantigens CMS2 vs CMS4 using Wilcoxon rank sum test with continuity correction: W = 5232.5, n_CMS2 = 136, n_CMS4 = 87, p = 0.217060840755576
more run.out | grep -i neoantigen | grep tcga-all | grep 'CMS2 vs CMS1' | grep -v msi | grep -v 'KRAS MT'
## tcga-all: Analyzing neoantigens CMS2 vs CMS1 using Wilcoxon rank sum test with continuity correction: W = 6531.5, n_CMS2 = 136, n_CMS1 = 51, p = 4.11558761110611e-21
