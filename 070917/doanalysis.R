...
plotKrasCMSAnalysis <- function(es.arg, clin.arg, kras.cms.tbl, to.plot=c(), ylabels=c(), analysis.name, kras.status.field="kras", kras.states=c("MT","WT"), plot.adjusted.pvalues = FALSE, plot.pvals.as.stars = FALSE, main = NULL) {
    ...
    to.plot <- intersect(to.plot, rownames(es))
...
plot.stacked.faceted.effect.size.figures <- function(tbl1, tbl2, tbl1.mle.col, tbl1.lb.col, tbl1.up.col, tbl2.mle.col, tbl2.lb.col, tbl2.ub.col, main1, main2, text.size = NULL) {
    ...
    tbl1$mle <- as.numeric(tbl1$mle)
    tbl1$low <- as.numeric(tbl1$low)
    tbl1$high <- as.numeric(tbl1$high)        
    ...
    tbl2$x <- unlist(apply(tbl2[, c("x", "cms", "kras")], 1,
                           function(row) as.numeric(row[1]) + as.numeric(mp[[paste0(row[2], row[3])]])))
    tbl2$mle <- as.numeric(tbl2$mle)
    tbl2$low <- as.numeric(tbl2$low)
    tbl2$high <- as.numeric(tbl2$high)        
