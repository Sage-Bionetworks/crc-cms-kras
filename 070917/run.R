...
tbl.neg <- subset(tbl, ( mle < 0 & !j1 ) | j2)
tbl.pos <- subset(tbl, ( mle > 0 & !j2 ) | j1)
g <- g + geom_text(data = tbl.pos, aes(x = mle, y = - log10(pval), label = gene.set), hjust = -0.1, show.legend = FALSE)
g <- g + geom_text(data = tbl.neg, aes(x = mle, y = - log10(pval), label = gene.set), hjust = 1.1, show.legend = FALSE)
...
tbl.neg <- subset(tbl, ( pval < 0.1 & mle < 0 & !j1 ) | j2)
tbl.pos <- subset(tbl, ( pval < 0.1 & mle > 0 & !j2 ) | j1)
## tbl.j1 <- tbl[j1,]
## tbl.j2 <- tbl[j2,]
...
pg <- plot_grid(g, g.hall, labels = c("A", "B"), nrow=1)
...
for(i in 1:length(ns)) {
    ...
    gene.set.order <- NULL
    if(name == "immune") {
        immune.levels <- c("Cytotoxic cells", "Neutrophils", "Th1 cells")
        tbl <- subset(tbl, gene.set %in% immune.levels)
        gene.set.order <- immune.levels
    }
    if(name == "hallmark") {
        hallmark.levels <- c("INTERFERON_GAMMA_RESPONSE", "INFLAMMATORY_RESPONSE", "IL6_JAK_STAT3_SIGNALING", "COMPLEMENT", "INTERFERON_ALPHA_RESPONSE", "P53_PATHWAY")
        ##        tbl <- subset(tbl, gene.set %in% c("INTERFERON_GAMMA_RESPONSE", "INTERFERON_ALPHA_RESPONSE", "P53_PATHWAY", "ALLOGRAFT_REJECTION", "IL6_JAK_STAT3_SIGNALING", "INFLAMMATORY_RESPONSE", "COMPLEMENT"))
        tbl <- subset(tbl, gene.set %in% hallmark.levels)
        gene.set.order <- hallmark.levels
    }
